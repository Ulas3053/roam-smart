...
const SearchPlaces = () => {
  ...
  const [rawWeather, setRawWeather] = useState(null); // original response
  const [weatherObj, setWeatherObj] = useState(null); // parsed inner weather
  ...
  const handleSearch = async () => {
    ...
    try {
      const [placesRes, weatherRes] = await Promise.all([
        axios.get(`/api/place-search/explore?place=${encodeURIComponent(place)}`),
      ...
      // The inner weather is a stringified JSON; parse it safely
      try {
  ...
  return (
    <div className="search-places-container">
      ...
      {weatherObj && (
        <div className="weather-box">
              ...
              <div className="weather-main">
                ...
                <div className="weather-desc">
                  <div className="temp">
                    ...
                    <span className="feels">Feels like {Math.round(weatherObj.main.feels_like)}°C</span>
                  </div>
                  <div className="desc-text">{weatherObj.weather[0].description}</div>
                </div>
              </div>
            )}
          </div>
          <div className="weather-details">
            ...
            <div>
              <strong>Wind:</strong> {weatherObj.wind.speed} m/s @{weatherObj.wind.deg}°
            </div>
            <div>
      ...
      {results.length > 0 && (
        <div className="itinerary-cta" style={{ textAlign: 'center', marginTop: '1.5rem' }}>
